...
namespace AutomationFoundation.Features.ProducerConsumer.Resolvers
{
    ...
    {
        private readonly Func<ProducerConsumerContext<TItem>, IProducer<TItem>> callback;
        ...
        public CallbackProducerResolver(Func<ProducerConsumerContext<TItem>, IProducer<TItem>> callback)
        {
            this.callback = callback ?? throw new ArgumentNullException(nameof(callback));
        }

        /// <inheritdoc />
        public IProducer<TItem> Resolve(ProducerConsumerContext<TItem> context)
        {
            if (context == null)
            {
